...
func TestAddDeleteSupplier(t *testing.T) {
	...
	_, err := http.Post("http://209.38.198.55:8082/suppliers/add", "application/json", bytes.NewBuffer(jsonStr))
	if err != nil {
	...
	_, err = http.Post("http://209.38.198.55:8082/suppliers/delete", "application/json", bytes.NewBuffer(jsonStr))
	if err != nil {
		t.Fatalf("The HTTP request failed with error %s\n", err)
	}
...
func TestShowDeliveries(t *testing.T) {
	resp, err := http.Get("http://209.38.198.55:8082/deliveries/show")
	if err != nil {
		t.Fatalf("The HTTP request failed with error %s\n", err)
	}
...
func getSuppliers() []Supplier {
	resp, err := http.Get("http://209.38.198.55:8082/suppliers/show")
	if err != nil {
		return nil
	}
	defer resp.Body.Close()
